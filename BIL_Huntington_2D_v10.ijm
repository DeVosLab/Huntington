...
/* 	VERSION SUPPORT: 
	...
	v10:- Save branch skeleton information
		- Verification tool skeleton
*/  
...
var analysisStack 	= true;
var analysisSubStack= true;
...
macro "[I] Install Macro"{
	...
	run("Install...", "install=[/data/CBH/mverschuuren/ACAM/CF-BIL_TamaraVasilkovska/200401_Huntington/GitHub/Huntington/BIL_Huntington_2D_v10.ijm]");
}
...
macro "Segment Markers Action Tool -C999 Hff1f1cfcff00"{
	...
	if(File.exists(dirOutput+name+"_CH"+chMarker1+"_Skeleton.zip")){
		roiManager("Open",dirOutput+name+"_CH"+chMarker1+"_Skeleton.zip");
		currentCount=roiManager("count");
		roiManager("Select", (currentCount-1));
		roiManager("Set Color", "yellow");
		roiManager("Show All without labels");
...
macro "Analyse Single Image Action Tool - C888 T5f161"{
	...
	if(analysisStack){
		...
		if(File.exists(dirOutput+prefix+"_CH"+chMarker1+"_Skeleton.zip")){
			roiManager("Open",dirOutput+prefix+"_CH"+chMarker1+"_Skeleton.zip");
			currentCount=roiManager("count");
			roiManager("Select", (currentCount-1));
			roiManager("Set Color", "yellow");
			roiManager("Show All without labels");
...
macro "Verification Marker Stack Action Tool - C888 T1f16V Taf07M "{
	...
	run("Channels Tool... ");
	Stack.setDisplayMode("composite");
	Stack.setActiveChannels("0110111");
}
...
function setup(){
	...
	Dialog.setInsets(0,0,0);
	Dialog.addMessage("Analysis blocks:\n",12,"#ff0000");
	Dialog.setInsets(0,100,0);
	Dialog.addCheckbox("Stack: marker1/2", analysisStack);
	Dialog.addToSameRow();
	Dialog.addCheckbox("SubStack: Nuclei, 2B4 and marker1/2", analysisSubStack);
	Dialog.setInsets(0,0,0);
	...
	analysisStack	= Dialog.getCheckbox();		print("Analysis Stack Marker1/2: ", analysisStack);
	analysisSubStack= Dialog.getCheckbox();		print("Analysis SubStack DAPI, 2B4, Marker1/2: ", analysisSubStack);
