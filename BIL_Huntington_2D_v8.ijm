...
macro "[I] Install Macro"{
	...
	run("Install...", "install=[/data/CBH/mverschuuren/ACAM/CF-BIL_TamaraVasilkovska/200401_Huntington/BIL_Huntington_2D_v8.ijm]");
}
...
function setup(){
	...
	Dialog.addToSameRow();
	Dialog.addNumber("Minimum Aspect Ratio ", minARExclude, 2, 5, "");
	Dialog.setInsets(0,0,0);
	Dialog.show();
	...
	minARExclude			= Dialog.getNumber();		print("Min Aspect Ratio Exclude:", minARExclude);
...
function segmentationSpots2D (id,name, booleanDebug){
	...
	if(excludeVesselStructures){
		...
		run("Analyze Particles...", "size=1-Infinity circularity=0-1.00 show=Nothing display clear include add");
		...
		for(i=0; i<roiManager("count"); i++){
			if(getResult("AR", i)>minARExclude){
			   selectImage(idMaskExclude);
				roiManager("select", i);
				setForegroundColor(255, 255, 255);
				run("Fill", "slice");	
